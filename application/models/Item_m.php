...
class item_m extends CI_Model {
    ...
    public function get($id = null) 
    {
        ...
        $this->db->where('p_item.user_id', $this->session->userdata('user_id')); // Filter berdasarkan user_id
        if($id != null) {
    ...
    public function get_by_user($user_id) 
    {
        ...
        $this->db->join('p_category', 'p_category.category_id = p_item.category_id');
        $this->db->join('p_unit', 'p_unit.unit_id = p_item.unit_id');
        $this->db->where('p_item.user_id', $user_id); // Assuming 'user_id' is a field in 'p_item' that associates items to users
        $this->db->order_by('barcode', 'asc');
        $query = $this->db->get();
        return $query;
    }
    ...
    public function add($post)
    {
        $params = [
            ...
            'price' => $post['price'],
            'image' => $post['image'],
            'user_id' => $this->session->userdata('user_id'), // Set user_id to the logged-in user's ID
        ];
